...
void rr_ui_level_select_delete(rrUILevelSelect* level_select) {
    rr_ui_active_element_group_uninit(&level_select->_element_group);
}

void rr_ui_level_select_show(rrUILevelSelect* level_select) {
    rr_ui_active_element_group_set(&level_select->_element_group, 0);
}
...
void rr_ui_level_select_update_active_level_text(rrUILevelSelect* level_select) {
    if (rr_input_button_down(level_select->_input, RR_INPUT_BUTTON_DOWN))
        rr_ui_active_element_group_next(&level_select->_element_group);
...
void rr_ui_level_select_update_active_buttons(rrUILevelSelect* level_select) {
    if (rr_input_button_down(level_select->_input, RR_INPUT_BUTTON_RIGHT))
        ...
        rr_ui_active_element_group_previous(&level_select->_element_group);
        ...
        rrUiButton * button = (rrUiButton*)rr_ui_active_element_group_get_current(&level_select->_element_group);
...
void rr_ui_level_select_center_content(rrUILevelSelect* level_select, rrRect* content_rect) {
    ...
    level_select->_layout_rect.w = content_rect->w + RR_UI_LEVEL_SELECT_PADDING * 2;
    level_select->_layout_rect.h = content_rect->h + RR_UI_LEVEL_SELECT_PADDING * 2;
    level_select->_layout_rect.x = screen_size.x / 2 - level_select->_layout_rect.w / 2;
    level_select->_layout_rect.y = screen_size.y / 2 - level_select->_layout_rect.h / 2;

    rr_point_set(&offset, level_select->_layout_rect.x + RR_UI_LEVEL_SELECT_PADDING, level_select->_layout_rect.y + RR_UI_LEVEL_SELECT_PADDING);

    level_select->bar.rect.x += level_select->_layout_rect.x + 4;
    level_select->bar.rect.y += level_select->_layout_rect.y + 5;
    level_select->bar.rect.w = level_select->_layout_rect.w - 8;
...
void rr_ui_level_select_layout(rrUILevelSelect* level_select) {
    ...
    rr_ui_active_element_group_add(&level_select->_element_group, &level_select->level_text.element);
    rr_ui_basic_sprite_get_rect(&level_select->level_prompt, &element_rect);
    rr_rect_encapsulate(&content_rect, &element_rect);
    position.y += text_sprite->rect.h + 10;
    ...
    rr_ui_active_element_group_add(&level_select->_element_group, &level_select->ok_button.element);
    rr_ui_basic_sprite_get_rect(&level_select->level_prompt, &element_rect);
    rr_rect_encapsulate(&content_rect, &element_rect);
    ...
    rr_ui_active_element_group_add(&level_select->_element_group, &level_select->cancel_button.element);
...
void rr_ui_level_select_draw(rrUILevelSelect* level_select) {
    ...
    rr_ui_draw_inset_rect(level_select->_renderer, &level_select->_layout_rect);
    rr_renderer_color(level_select->_renderer, &color);
