...
void rr_sdl_display_update(rrSDLDisplay* display) {
    switch (display->display_screen) {
        case RR_SCREEN_GAME: {
            ...
            if (display->game_ui->_previous_state == RR_GAME_STATE_OVER &&
            display->game_ui->high_score_dialog->active == 0 &&
            (rr_input_button_down(display->input, RR_INPUT_BUTTON_START) || rr_input_button_down(display->input, RR_INPUT_BUTTON_ACCEPT)))
                rr_sdl_display_set_screen(display, RR_SCREEN_TITLE);
